...
class FeatureTemplates {
  ...
  static String binding(String featureNamePascal, String featureNameSnake) {
    ...
    class ${featureNamePascal}Binding extends Bindings {
      @override
      void dependencies() {
        Get.lazyPut<I${featureNamePascal}Repository>(() => ${featureNamePascal}Repository());
        Get.lazyPut<${featureNamePascal}Controller>(() => ${featureNamePascal}Controller());
      }
    }''';
  }
  ...
  static String controller(String featureNamePascal, String featureNameSnake) {
    ...
    class ${featureNamePascal}Controller extends GetxController {
  ...
  static String page(String featureNamePascal, String featureNameSnake) {
    ...
    class ${featureNamePascal}Page extends GetView<${featureNamePascal}Controller> {
      ...
      Widget build(BuildContext context) {
        return Scaffold(
          appBar: AppBar(
            title: Text(${featureNamePascal}Keys.pageTitle),
  ...
  static String repository(String featureNamePascal, String featureNameSnake) {
    ...
    abstract class I${featureNamePascal}Repository {
      ...
      /// Envia dados
      Future<Either<String, bool>> sendData(Map<String, dynamic> data);
      
      /// Atualiza dados
      Future<Either<String, bool>> updateData(String id, Map<String, dynamic> data);
      
      /// Remove dados
      Future<Either<String, bool>> deleteData(String id);
    }

    /// Implementação do repositório $featureNamePascal
    class ${featureNamePascal}Repository implements I${featureNamePascal}Repository {
      // Implementar cliente HTTP aqui
      // final HttpClient _client = Get.find();
      
      final String _basePath = '/api/$featureNameSnake';

      @override
      Future<Either<String, Map<String, dynamic>>> getData() async {
        try {
          ...
          return Left('Erro ao obter dados: \$e');
        }
      }

      @override
      Future<Either<String, bool>> sendData(Map<String, dynamic> data) async {
        try {
          // Implementar chamada HTTP POST
          // final response = await _client.post('\$_basePath', data);
          
          await Future.delayed(const Duration(seconds: 1));
          return const Right(true);
        } catch (e) {
          return Left('Erro ao enviar dados: \$e');
        }
      }

      @override
      Future<Either<String, bool>> updateData(String id, Map<String, dynamic> data) async {
        try {
          // Implementar chamada HTTP PUT
          // final response = await _client.put('\$_basePath/\$id', data);
          
          await Future.delayed(const Duration(seconds: 1));
          return const Right(true);
        } catch (e) {
          return Left('Erro ao atualizar dados: \$e');
        }
      }

      @override
      Future<Either<String, bool>> deleteData(String id) async {
        try {
          ...
          return Left('Erro ao remover dados: \$e');
        }
      }
    }''';
  }
  ...
  static String keys(String featureNamePascal, String featureNameSnake) {
    ...
    class ${featureNamePascal}Keys {
      ...
      // Keys para tradução (usar com sistema de i18n)
  ...
  static String translate(String featureNamePascal, String featureNameSnake) {
    return '''class ${featureNamePascal}Translate {
      static const String _prefix = '$featureNameSnake';
      
      /// Traduções em Português
      static const Map<String, String> pt_BR = {
      ...
      static const Map<String, String> es_ES = {
        '\${_prefix}_page_title': '${ReCase(featureNamePascal).titleCase}',
